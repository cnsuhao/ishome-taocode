...
### 13.2 成绩表管理页面接口
####13.2.1 通过考试标签查询班级列表查询接口
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/results/classlist?resultsTagId=[resultsTagId]&tid=[tid]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - 'resultsTagId'：         成绩标签id--必填项
    - 'tid'：                  教师id--可选项，不传或为0则返回该学期所有班级；传tid则获取该id下的班级
    - 'page':                  分页编号，分页为0或者空时，返回第一页数据
    - 'size':                  每页数据条数，为0或空时，默认返回12条数据
    - 'token'：                token
Response:
...
```
####13.2.1-1 通过考试标签查询班级列表（班级列表）查询接口
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/results/classlist/info?resultsTagId=[resultsTagId]&tid=[tid]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - 'resultsTagId'：         成绩标签id--必填项
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/results/classlist/search?&cid=[cid]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - 'cid'：                  班级id--可选项
