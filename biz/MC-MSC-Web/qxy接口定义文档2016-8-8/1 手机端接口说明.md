...
``` json
Url:        /qxy/schedule/mobile/week/term=[term_id]&week=[week]&teacher=[tid]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `term`:        指定查询的学期id，term为空时查询缺省的学期
    - `week`:        指定该学期的对应周，当数据为空时，选择当前日期所在的周
    - `teacher`:     指定教师id，当数据为空时默认获取token中包含tid的值周数据
Response:
    - `status`:      0->OK 
					 1->获取失败，查找的周不存在
                     10->token is invalid
...
``` json
Url:        /qxy/schedule/mobile/day/term=[term]&day=[date]&teacher=[tid]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `term`:        指定查询的学期id，term为空时查询缺省的学期
    - `day`:         指定该学期的日期，当数据为空时，选择当前日期
    - `teacher`:     指定教师id，当数据为空时默认获取token中包含tid的值日数据
Response:
    - `status`:      0->OK 
					 1->获取失败，查找的日不存在
                     10->token is invalid
