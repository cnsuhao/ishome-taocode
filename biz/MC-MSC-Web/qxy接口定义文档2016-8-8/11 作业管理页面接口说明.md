...
``` json
Url:        /qxy/task/class?tid=[tid]&cid=[cid]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - 'tid'：   tid＝0，或者为空，查询班级全部教师
    - `cid`:班级id
    - `page` 不传默认为1
    - `size` 不传默认为12
    - `token` ：      token
...
``` json
Url:        /qxy/task/teacher?tid=[tid]&cid=[cid]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - 'cid'：   cid为空，为老师所在的全部班级
    - `tid`:教师id
    - `page` 不传默认为1
    - `size` 不传默认为12
    - `token` ：      token
...
``` json
Url:        /qxy/task/search?courseId=[courseId]&taskName=[taskName]&tid=[tid]&cid=[cid]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - 'courseId'：   作业栏目 
    - `taskName`:    作业标题
    - 'cid'：   cid为空，为老师所在的全部班级（二级查询下，cid必填）
    - `tid`:教师id ，      tid＝0，或者为空，查询班级全部教师
