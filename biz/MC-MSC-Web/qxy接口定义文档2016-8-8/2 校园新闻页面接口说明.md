...
``` json
Url:        /qxy/news?column=[colId]&time=[startTime|endTime]&type=[type]&teacher=[tid]&page=[page]&size=[size]&token=[token]
Method:     GET
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/news/detaile?news=[newId]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - 'newsId'：       新闻id
Response:
    ...
    - 'newsReaders'： 新闻教师阅读范围，为空表示全部
    - 'newsClassers'：新闻班级阅读范围，为空表示全部
    - 'isTop'：      是否置顶0-否，1-是
    - 'publishTime': 发布时间
...
####2.1.3 新闻新增接口
> 新闻新增接口提供了对新闻的新增功能（注意：不需要审核的新闻发布后是无法修改的，只能删除后重新发布）（注意：显示新闻的时候只要在newsReaders及newsClassers范围内的用户都能看到本新闻）
...
示例：
{
    ...
    "columnId":"1",
    "newsType":"2",
    "newsReaders":"234;211;332;",       #可阅读的教师tid集合
    "newsClassers":"222;211;233;",      #可阅读的班级cid集合
...
> （注意：不需要审核的新闻发布后是无法修改的，只能删除后重新发布）（注意：显示新闻的时候只要在newsReaders及newsClassers范围内的用户都能看到本新闻）
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/news
Method:     PUT
    ...
    "newslist":[
	    {
		    ...
		    "columnId":"1",
		    "newsType":"2",
		    "newsReaders":"222;211;233;",       #tid
		    "newsClassers":"222;211;233;",      #cid
		    "publishTime":"2016-8-9 00:00:00",
		    "isPic":"0",
		    "pic":"url",
            "isAudit":"1",         #0-未审核，1-审核通过，2-审核驳回
...
####2.2.2 新闻栏目新增接口
> 栏目新增接口提供了对栏目的新增功能（注意：显示新闻的时候只要在newsReaders及newsClassers范围内的用户都能看到本新闻）
...
> 栏目修改及停用(启用)接口提供了对栏目的修改及停用(启用)功能（注意：显示新闻的时候只要在newsReaders及newsClassers范围内的用户都能看到本新闻）
