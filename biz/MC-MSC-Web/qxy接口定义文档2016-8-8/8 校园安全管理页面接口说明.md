...
``` json
Url:        /qxy/securitylog?studentId=[studentId]&deviceId=[deviceId]&time=[startTime|endTime]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
...
``` json
Url:        /qxy/class/alarmlist?page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `token`:         token
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/studentrfidlist?page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
