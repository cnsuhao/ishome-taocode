...
public class HoughLineFilter extends AbstractBufferedImageOp{
	...
	public HoughLineFilter(){
		//default hought ransform parameters
		//threshold=0.5f;
		//scale=1.0f;
		//offset=0.0f;
		threshold=1.0f;
...
//	@Override
	...
	private void houghTransform(int[] inPixels,int[] outPixels){
		...
		//define temp hough2D array and initialize the hough2D
		...
		for(int row=0; row < height; row++){
			for(int col=0; col < width; col++){
				// this value need to be considered .  lhl
				// to avoid all point have been calculated
//				int p=image_2d[row][col] & 0xff;  
//				if(p==0) continue;//which means background color
//				if (image_2d[row][col] > -11776961) continue;
				ColorModel cm = ColorModel.getRGBdefault(); 
				if (  ( cm.getRed(image_2d[row][col]) + cm.getRed(image_2d[row][col]) + cm.getRed(image_2d[row][col]) ) > 300)  {
						continue; 
					}
...
//							image_2d[subrow][subcol]=-16776961;
							int tmp = image_2d[subrow][subcol];
							if (image_2d[subrow][subcol] != -1) image_2d[subrow][subcol]=-16776961;
						}
					}
				}else{
		...
		{
			int value=clamp((int)(scale*this.hough_1d[i]+offset));//scalealwaysequals1
	...
	public BufferedImage filterBycolor(BufferedImage src,BufferedImage dest){
		width=src.getWidth();
		height=src.getHeight();
		...
		for(int i=0; i<inPixels.length; i++)
		{
			isCount = false;
			for (int j=0; j<num; j++){
				if (( inPixels[i] <= color[j]+3000) && ( inPixels[i] >= color[j] -3000 ) ) {
					colorcount[j] ++;
					isCount = true;
					break;
				}
