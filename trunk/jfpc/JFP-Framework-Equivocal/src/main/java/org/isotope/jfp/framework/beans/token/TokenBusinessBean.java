...
public class TokenBusinessBean extends UserBean implements ISFrameworkConstants {
	...
	public static String getBizToken(String companyId, String userId, String bizName, String encryType, String clientTimestamp) {
		return TokenBusinessHelper.getBizTokenData(companyId, userId, bizName + encryType + clientTimestamp);
	}
	...
	public static String[] getBizTokenData(String userToken) {
		...
		String[] token = TokenBusinessHelper.getBizTokenData(userToken);
	 ...
	 */
	public static TokenBusinessBean build(String bizToken) {
	 ...
	 * @return tonkenString
	 */
	public static String getBizToken(TokenBusinessBean bizTokenBean) {
		return TokenBusinessHelper.getBizTokenData(bizTokenBean.getUserId(), bizTokenBean.getCompanyId(), bizTokenBean.getBizName() + bizTokenBean.getEncryType() + bizTokenBean.getClientTimestamp());
	}
	...
	public String getToken() {
		if (EmptyHelper.isEmpty(token))
			token = TokenBusinessHelper.getBizTokenData(companyId, userId, bizName + encryType + clientTimestamp);
		return token;
	}
