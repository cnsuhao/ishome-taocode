...
> 本班级对应的学生简单情况；
``` json
Url:        /qxy/stundent/namelist?class=[cid]&token=[token]
Method:     GET
...
``` json
Url:        /qxy/securitylog?student=[studentId]&device=[deviceId]&time=[startTime|endTime]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
...
``` json
Url:        /qxy/class/alarmlist?cid=[cid]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `term`:        查询的学期id,不传为缺省学期
    - `grade`:       查询年级id
    - `class`:       查询的班级cid  当输入班级id时可不传分页参数
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/studentrfidlist?student=[studetId]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `term`:   学期id
