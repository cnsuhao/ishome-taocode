...
``` json
Url:        /qxy/securitylog?termId=[termId]&studentId=[studentId]&deviceId=[deviceId]&time=[startTime|endTime]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/studentrfidlist?termId=[termId]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `termId`: 学期id，可不传则获取所有列表
