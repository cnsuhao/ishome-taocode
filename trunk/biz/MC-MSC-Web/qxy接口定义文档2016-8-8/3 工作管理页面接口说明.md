...
Header:     Content-type:application/json
Parameter:
    - `ids`:  要删除的工作安排id集合,对应sDutyScheduling表中自增id
    - 'token'：       token
示例：
{
    "ids":[1,2,3]
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/schedule/copy?token=[token]
Method:     PUT
Header:     Content-type:application/json
Parameter:
    - 'weeks':        周信息
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/duty/content?token=[token]
Method:     POST
Header:     Content-type:application/json
Parameter:
    - `dutyId`:     岗位id
...
```
#### 3.4.1-1 我提交的事项详情查询接口
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/myapplication/info?approvalInformationId=[approvalInformationId]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
	- 'approvalInformationId' :        提交事项id
    - 'token' ：      token
Response:
        - `status`: 0->is OK 
                    10->token is invalid
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/myapplication?token=[token]
Method:     POST
Header:     Content-type:application/json
Parameter:
        - `token`:        token
...
```
#### 3.4.4 我的审批事项查询接口
> 我的审批查询接口用于查询我参与进行的审批工作流列表，接口使用tid来查询具体教师发布的工作流。工作流目前分为三类（请假，活动，报修）。审批列表通过查询数据库oa_examine_result的内容来形成结果。其中仅仅显示tid对应的数据，并且该数据还是生效的数据。（仅显示表中ruleStatus字段为0的数据内容，为1则表示该规则以及失效，为9时该审批请求已经被删除）
>
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/myaudit?tid=[tid]&page=[page]&size=[size]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
        - 'tid' : 需要查询的教师ID，没有传参数的查询本人（token中包括）
...
Response:
        - `status`: 0->is OK 
                    ...
                    13->Illegal interface calls, not power use interface
        "id":               #key,审批表的id
        "approvalInformationId":  #对应的用户发起的审批id
        "type":   #0-请假；1-活动；2-保修
        "applyContent":  #申请内容
        "content":  #审批内容
        "createTime":  #申请流程时间
...
```
#### 3.4.4-1 我的审批事项详情查询接口

> 需要使用Token验证查询者身份
``` json
Url:        /qxy/myaudit/info?id=[id]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
        - id : 需要查询的审批事项id --必填参数
Response:
		...
		"createTime":              #申请流程时间
		"oatagsId":                #应用的流程模版
		"oatagsName":              #应用的流程模版名称
		"oaruleStatus":            #应用流程规则状态 0-未完成；1-完成
		"tid":                     #发起人的tid
		"teacherName":              #发起人的名字
```
...
``` json
Url:        /qxy/myaudit/set?token=[token]
Method:     PUT
Header:     Content-type:application/json
Parameter:
    - `id` :  我的审批表中的key
    - `result` : #1-申请通过；2-申请驳回
    - `content` :   审批内容
    - `token`: 用户token
示例：
{
