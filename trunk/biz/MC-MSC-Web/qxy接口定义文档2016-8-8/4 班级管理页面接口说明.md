...
``` json
Url:        /qxy/classlist?termId=[termId]&tid=[tid]&page=[page]&size=[size]&token=[token]
Method:     GET
...
Parameter:
    ...
    - `termId`:  学期id
    - `token`:   token
    - `tid`:     用户为管理员时不需要传获取所有班级，用户为教师时需要传tid获取自己的班级
Response:
    - `status`： 0->OK 
                1->学期数据不正确
                10->token is invalid
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/class/course?token=[token]
Method:     POST
Header:     Content-type:application/json
Parameter:
    - `courseId`:  课程id
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/class/teacher?token=[token]
Method:     PUT
Header:     Content-type:application/json
Parameter:
    - `id`:    #class_teacher表中自增id
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/class/student/info?token=[token]
Method:     POST
Header:     Content-type:application/json
Parameter:
    - 'cid'：        班级id
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/class/student/parent?token=[token]
Method:     POST
Header:     Content-type:application/json
Parameter:
    - 'token'：      token
...
> 需要使用Token验证查询者身份
``` json
Url:        /qxy/student/move?token=[token]
Method:     PUT
Header:     Content-type:application/json
Parameter:
    - `cid`:         班级id
