...
``` json
Url:        /qxy/classlist/term=[termId]&grade=[gradeId]&class=[cid]&page=[page]&size=[size]&teacher=[tid]&token=[token]
Method:     GET
...
``` json
Url:        /qxy/class/student/info/cid=[cid]&student=[studentId]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `studentId`:    查询的学生id
...
####4.4.2 学生(家长)列表查询接口
> 查询学生列表，显示家长学生关联表中isDefault为1的家长

> 需要使用Token验证查询者身份

``` json
Url:        /qxy/stundent/list/term=[termId]&grade=[gradeId]&class=[cid]&page=[page]&size=[size]&teacher=[tid]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `term`:   学期id
...
``` json
Url:        /qxy/student/info/term=[term]&grade=[grade]&class=[cid]&student=[studentId]&teacher=[tid]&token=[token]
Method:     GET
Header:     Content-type:application/json
Parameter:
    - `term`:   学期id
