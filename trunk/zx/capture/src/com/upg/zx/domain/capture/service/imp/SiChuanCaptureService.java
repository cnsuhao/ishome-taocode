...
public class SiChuanCaptureService extends CaptureServiceImp {
...
//		map.put("marketName", "请输入企业、农民专业合作社名称、个体工商户名称字号"); 
	...
	protected Map<String, String> getRequestParam(String corpId,RequestInfo requestinfo){
		...
		if("4".equals(requestinfo.getModeType())){
			//用于异常信息获取修改
			if(corpIdMap.get("2") == null || "".equals(corpIdMap.get("2"))){
				params_map.remove("maent.entbigtype");
				params_map.put("czmk", "czmk1");
				params_map.put("from", "");
			}else{
				params_map.put("djjg", "");	
			}
